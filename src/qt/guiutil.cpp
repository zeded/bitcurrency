...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("bitcurrency"))
        return false;
...
{
    // Convert bitcurrency:// to bitcurrency:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("bitcurrency://"))
    {
        uri.replace(0, 12, "bitcurrency:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BitCurrency.lnk";
}
...
{
    return GetAutostartDir() / "bitcurrency.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("BitCurrency-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  bitcurrency-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("BitCurrency-Qt"));
